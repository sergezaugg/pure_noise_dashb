...
with col_a:
    ...
    At each run, the dataset will be split 50/50 int0 train and test set.
    The simulation results are accumulated in memory and shown in the tabs below.
    Note tha each time you run a simulation a new dataset is randomly sampled from the scenario.
    Thus you can do multiple monte-carlo simulations (recommended).
    ''')
    ...
    Raising max-feature to 10 considerably improved the robustness to pure-noise-features.
    Caution, these results do not directly translate to real-world datasets.
    But it is interesting to see that in certain situations, RF can handle dozens of pure-noise-features without any loss in predictive performance.
