...
if len(ss['resu']) > 0:
    ...
    with st.expander("(3) Check simulation results (click to expand/collapse)", expanded=True):
        ...
        with CB:
            ...
            _ = st.slider("Width",  min_value=100, max_value=1500, value=1000, step=100, key="wid10", on_change=update_ss, args=["wid10", "par10"])
            _ = st.slider("Height", min_value=100, max_value=1500, value=500, step=100, key="wid11", on_change=update_ss, args=["wid11", "par11"])
